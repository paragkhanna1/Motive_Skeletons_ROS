...
int main(int argc, char **argv)
{
 ...
 geometry_msgs::TransformStamped transform_skeleton2[13];
  ...
  while (ros::ok())
  {
    ...
    if (data_cp !=NULL)
    {
      ...
      printf("Rigid Bodies [Count=%d]\n", data_cp->nRigidBodies);
        ...
        if (i==0)
        {
          ...
          transformStamped.transform.translation.y = data_cp->RigidBodies[i].y;
          transformStamped.transform.translation.z = data_cp->RigidBodies[i].z;

          transformStamped.transform.rotation.x = data_cp->RigidBodies[i].qx;
          transformStamped.transform.rotation.y = data_cp->RigidBodies[i].qy;
          transformStamped.transform.rotation.z = data_cp->RigidBodies[i].qz;
    	...
    	for(i=0; i < data_cp->nSkeletons; i++)
    	{
          ...
          transform_skeleton1[j].transform.translation.y = rbData.y;
          transform_skeleton1[j].transform.translation.z = rbData.z;

          transform_skeleton1[j].transform.rotation.x = rbData.qx;
          transform_skeleton1[j].transform.rotation.y = rbData.qy;
          transform_skeleton1[j].transform.rotation.z = rbData.qz;
    ...
    wrench33.header.stamp = transformStamped.header.stamp;
